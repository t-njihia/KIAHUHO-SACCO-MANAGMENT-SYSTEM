...
while ($SavingResult = $SavingQuery->fetch_assoc()) {
	$totalSaving += $SavingResult['amount'];
			...
			Total Savings: <font color="green" style="font:bold 22px 'Aleo';"><?php echo $totalSaving;?></font>
			</div>
</div>
<input type="text" name="filter" style="padding:15px;" id="filter" placeholder="Search Member..." autocomplete="off" />
<a rel="facebox" href="addsavings.php"><Button type="submit" class="btn btn-info" style="float:right; width:230px; height:35px;" /><i class="icon-plus-sign icon-large"></i> Add New Savings</button></a><br><br>
...
accounts.customer_id=tbl_savings.customer_id ORDER BY tbl_savings.customer_id DESC");
			...
			<td>
			<a href="#" id="<?php echo $row['tbl_savings_id']; ?>" class="delbutton" title="Click To Delete"><button class="btn btn-danger btn-mini"><i class="icon-trash"></i> Delete</button></a></td>
			</tr>
			<?php
				}
			?>
